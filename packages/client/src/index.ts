...
async function getMockData<T>(cmd: string, args?: Record<string, unknown>): Promise<T> {
  ...
  const mockMap: Record<string, unknown> = {
    ...
    get_session_turns: { turns: mocks.MOCK_TURNS, eventsFileSize: 1024, eventsFileMtime: Date.now() } as TurnsResponse,
    check_session_freshness: { eventsFileSize: 1024, eventsFileMtime: Date.now() } as FreshnessResponse,
