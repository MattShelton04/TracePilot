/**
 * Per-section renderers for the version-analyzer markdown report.
 *
 * Each renderer is a pure function from report inputs to a `string[]` of
 * markdown lines. `generateMarkdownReport` composes them in order — see
 * `../presentation.ts`. Shared aggregation helpers and the `ReportContext`
 * type live in `./sections-context.ts`.
 */
...
import type { EventObservation, SessionVersionInfo } from "./types.js";
...
export function renderHeader(ctx: ReportContext): string[] {
  const { versions, sessionInfo, options } = ctx;
  const lines: string[] = [];
...
export function renderExecutiveSummary(ctx: ReportContext): string[] {
  const { versions, coverage, sessionInfo } = ctx;
  const lines: string[] = [];
...
export function renderInstalledVersions(ctx: ReportContext): string[] {
  const lines: string[] = [];
...
export function renderSchemaEvolution(ctx: ReportContext): string[] {
  const lines: string[] = [];
  ...
  for (const diff of ctx.diffs) {
    ...
    if (diff.removedEvents.length > 0) {
      ...
      for (const e of diff.removedEvents) lines.push(`- \`${e}\``);
      lines.push("");
    }
...
export function renderCoverageGap(ctx: ReportContext): string[] {
  const { coverage } = ctx;
  const lines: string[] = [];
...
export function renderSessionsByVersion(ctx: ReportContext): string[] {
  const lines: string[] = [];
  lines.push("## 5. Sessions by Copilot Version");
  lines.push("");
  const byVersion = new Map<string, SessionVersionInfo[]>();
  for (const s of ctx.sessionInfo) {
...
export function renderRpcMethods(ctx: ReportContext): string[] {
  const { versions } = ctx;
  const lines: string[] = [];
...
export function renderAgents(ctx: ReportContext): string[] {
  const { versions } = ctx;
  const lines: string[] = [];
...
export function renderEmpiricalObservations(ctx: ReportContext): string[] {
  const { diffs, coverage, empiricalStats, schemaEvents, unknownObserved } = ctx;
  const lines: string[] = [];
  ...
  if (unknownObserved.length > 0) {
    ...
    lines.push("");
    for (const eventType of unknownObserved.slice().sort()) {
      const stat = empiricalStats.get(eventType);
      lines.push(
...
export function renderLiveStreamOpportunities(ctx: ReportContext): string[] {
  const { diffs, schemaEvents } = ctx;
  const lines: string[] = [];
...
export function renderForwardCompat(_ctx: ReportContext): string[] {
  const lines: string[] = [];
...
export function renderRecommendations(ctx: ReportContext): string[] {
  const { coverage } = ctx;
  const lines: string[] = [];
