...
export function useSearchClipboard() {
  ...
  async function copyResultsToClipboard(items: SearchResult[]): Promise<boolean> {
    ...
    try {
      ...
      logWarn("[useSearchClipboard] Failed to copy results to clipboard", { count: items.length }, e);
      return false;
    }
  }
  ...
  async function copySingleResult(result: SearchResult): Promise<boolean> {
    try {
      ...
      logWarn("[useSearchClipboard] Failed to copy single result to clipboard", { sessionId: result.sessionId }, e);
      return false;
    }
  }
