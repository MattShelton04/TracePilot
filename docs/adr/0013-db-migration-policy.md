...
The cleanup plan called for unifying both runners behind a shared `Migrator`,
standardising on one `schema_version` convention, adding pre-migration backups,
and documenting the policy in an ADR.
