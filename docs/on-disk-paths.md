...
| `~/.copilot/tracepilot/` | TracePilot | App-owned local state colocated under Copilot home by default. | `TracePilotConfig.paths.tracepilotHome`, task DB, templates, presets, repo registry, backups, docs. | Path shape comes from `TracePilotPaths`. Users can change this app data directory in setup or Settings; TracePilot copies known app-owned files/directories into the new root and preserves the old root. |
...
| `~/.copilot/tracepilot/tasks.db` | TracePilot | AI task/orchestrator SQLite DB. | `TracePilotConfig::task_db_path`, Tauri task helpers. | Derived from `tracepilotHome`; copied with SQLite sidecars when the app data root changes. The shared DB handle reopens if `tracepilotHome` changes at runtime. |
...
| `~/.copilot/tracepilot/presets/` | TracePilot | Tauri-configured task presets. | `TracePilotConfig::presets_dir`, task preset commands. | Derived from `tracepilotHome`; copied recursively on app data root change. |
...
| `~/.copilot/tracepilot/jobs/` | TracePilot | File-based orchestrator IPC: manifest, heartbeat, task/context files. | `TracePilotConfig::jobs_dir`, task orchestrator commands. | Derived from `tracepilotHome`; copied recursively on app data root change, but still treated as transient runtime state. |
...
| `tracepilotHome` | TracePilot | Copy known app-owned files (`index.db`, `tasks.db`, sidecars, repo registry) and directories (backups, templates, presets, task-presets, jobs) into the new root if absent. Preserve old data by default; cleanup should be an explicit future action with confirmation. Surface errors if copy fails. |
