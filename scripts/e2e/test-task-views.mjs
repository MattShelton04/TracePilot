...
import { connect, navigateTo, startConsoleCapture, shutdown } from './connect.mjs';
...
async function run() {
  ...
  for (const route of ROUTES) {
    ...
    const warnings = capture.getLogs().filter(l =>
      l.text.includes('[ipc:SLOW]') || l.text.includes('not allowed')
    );
    ...
    console.log(`  Overflow: ${overflow.hasHorizontalOverflow ? 'YES ❌' : 'No ✅'} (${overflow.scrollWidth}/${overflow.clientWidth})`);
  ...
  console.log(`  Stats: ${dashboardInfo.statCount}, Orch card: ${dashboardInfo.hasOrchCard}, Preset cards: ${dashboardInfo.presetCardCount}`);
  ...
  console.log(`  Cards: ${presetsInfo.cardCount}, Gap: ${presetsInfo.gridGap}, Columns: ${presetsInfo.gridTemplateColumns}`);
