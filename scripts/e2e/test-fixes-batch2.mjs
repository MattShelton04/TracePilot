...
async function hasCssRule(page, selectorSubstr, propName, propValueSubstr) {
  return page.evaluate(({ sel, prop, val }) => {
...
await test('Preset auto-advances to Configure step', async () => {
  const onConfigStep = await page.evaluate(() => {
    const el = document.querySelector('.step-active, .wizard-step.active, [class*="step"][class*="active"]');
    if (el && el.textContent?.includes('Configure')) return true;
