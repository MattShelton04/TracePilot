...
try {
  ...
  const taskErrors = consoleAfterTasks.filter(
    (l) => l.type === "error" && (l.text.includes("task_list") || l.text.includes("not allowed")),
  );
  log(
    "No task IPC errors",
    taskErrors.length === 0,
  ...
  const firstSessionText = await page
  ...
  const orchContent = await page
    .locator('[data-testid="orchestration-actions"], .page-content')
    .count();
  ...
  if ((await firstCard.count()) > 0) {
    ...
    const detailContent = await page
      .locator('[data-testid="session-detail-content"], [data-testid="session-tabs"]')
      .count();
  ...
  const taskIpcCalls = ipcLog.filter((l) => l.cmd?.includes("task") || l.cmd?.includes("preset"));
  log(
    "Task IPC calls succeed",
    taskIpcCalls.length > 0,
  ...
  log(
    "No critically slow task IPC calls",
    slowCalls.length === 0,
    slowCalls.length > 0
      ? `slow: ${slowCalls.map((c) => `${c.cmd}=${c.duration}ms`).join(", ")}`
      : "all task cmds < 2s",
  );
  ...
  let presets;
  try {
    presets = await ipc(page, "task_list_presets");
  } catch {
    presets = null;
  }
    ...
    const step2Active = await page
      ...
      .evaluate(
        (el) =>
          ...
          el.parentElement?.classList.contains("completed"),
      )
      .catch(() => false);
    const formFields = await page
      .locator('.form-group, .config-section, input[type="text"], input[type="date"]')
      .count();
    log(
      "Preset auto-selected from URL",
      step2Active || formFields > 0,
