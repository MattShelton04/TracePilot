...
async function run() {
  ...
  await test('Section order: Active Tasks before Active Subagents', async () => {
    ...
    if (atIdx >= asIdx) throw new Error(`Active Tasks (${atIdx}) should come before Active Subagents (${asIdx})`);
  });
  ...
  await test('Section order: Activity Feed before Health & Recovery', async () => {
    ...
    if (afIdx >= hrIdx) throw new Error(`Activity Feed (${afIdx}) should come before Health & Recovery (${hrIdx})`);
  });
