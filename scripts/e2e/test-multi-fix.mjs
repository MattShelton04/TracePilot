...
async function main() {
  ...
  ({ browser, page, context, port } = await connect());
  ...
  await test('Tasks page loads without errors', async () => {
    ...
    const errors = capture.getLogs().filter(l => l.type === 'error' && !l.text.includes('favicon'));
    capture.stop();
    if (errors.length > 0) throw new Error(`Console errors: ${errors.map(e => e.text).join('; ')}`);
  });
  ...
  await test('Stat cards use CSS variables for colors', async () => {
    ...
    if (hardcoded.length > 0) throw new Error(`Hardcoded hex in stat cards: ${hardcoded.join(', ')}`);
  });
  ...
  await test('Task detail loads without null flash', async () => {
    ...
    const title = await page.locator('.detail-title').textContent().catch(() => null);
  ...
  await test('Orchestrator monitor loads', async () => {
    ...
    await page.waitForTimeout(2000);
    const label = await page.locator('.status-label').textContent().catch(() => null);
  ...
  await test('State color uses CSS variables', async () => {
    const result = await page.evaluate(() => {
      ...
      return { stateColor, hasClass: container.classList.contains('state-stopped') || container.classList.contains('state-healthy') || container.classList.contains('state-stale') || container.classList.contains('state-active') };
    });
  ...
  await test('TaskStatusBadge uses CSS variables', async () => {
    ...
    if (result.bad?.length > 0) throw new Error(`Hardcoded hex in badges: ${result.bad.join(', ')}`);
  });
  ...
  await test('No IPC errors during test', async () => {
    ...
    const errors = capture.getLogs().filter(l =>
      l.type === 'error' &&
      !l.text.includes('favicon') &&
      !l.text.includes('not found')
    );
    capture.stop();
