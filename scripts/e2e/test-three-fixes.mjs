...
await page.evaluate(() => { window.location.hash = '#/tasks/presets'; });
...
await test('Fix 2: Add source shows schema-driven fields', async () => {
  ...
  const addBtn = page.locator('button').filter({ hasText: /Add Source/ }).first();
...
await test('Fix 2: Changing type to multi_session_digest shows its config', async () => {
  ...
  assert(desc.includes('time window') || desc.includes('digest'), `Description doesn't match: ${desc}`);
});
