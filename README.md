...
View every user ↔ assistant exchange with full tool call details.
...
Concise charts provide dashboards for understanding your Copilot usage:

- **Analytics Dashboard** — token usage over time, sessions per day, cost trends, model distribution
- **Tool Analysis** — invocation frequency, success rates, and a day × hour activity heatmap
...
| Tab | What it shows |
|-----|--------------|
| **Overview** | Stat cards, session metadata, sessionp plan, checkpoints, incidents |
...
| **Todos** | Visualize copilots internal Task tracker with built-in dependency graph visualization capability |
...
- **Copilot Cost** — premium request count x cost per request (configurable, default $0.04)
- **Wholesale Cost** — what the same usage would cost via direct API access, using per-model token pricing

Pre-populated current prices cover Claude (Opus, Sonnet, Haiku), GPT (5.4, 5.1, 4.1, Codex), and Gemini models.
