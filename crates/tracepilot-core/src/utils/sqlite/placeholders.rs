...
pub fn build_in_placeholders(n: usize) -> String {
    ...
    // Optimization: First element is "?" (1 char) and rest are ", ?" (3 chars) -> `n*3 - 2` max capacity.
    // Unrolling the first iteration avoids checking `if i > 0` inside the loop.
    // Replacing individual pushes with a single chunk `push_str` reduces overhead.
...
pub fn build_placeholder_sql(sql_prefix: &str, num_rows: usize, params_per_row: usize) -> String {
    ...
    // Optimization: Unroll first push, and use `push_str` chunking instead of individual char pushes
