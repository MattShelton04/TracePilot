...
fn verified_skill_context_user_message_is_folded_into_invocation() {
    ...
    assert_eq!(
        skill_event.skill_invocation_id.as_deref(),
        Some("evt-skill")
    );
    assert_eq!(skill_event.skill_name.as_deref(), Some("trace-skill"));
    assert_eq!(
        skill_event.skill_path.as_deref(),
        Some("C:\\skills\\trace-skill\\SKILL.md")
    );
    assert_eq!(
        skill_event.skill_description.as_deref(),
        Some("Helpful skill")
    );
    assert_eq!(
        skill_event.skill_content_length,
        Some(skill_content.chars().count())
    );
    assert_eq!(skill_event.skill_context_length, Some(context_len));
    assert_eq!(skill_event.skill_context_folded, Some(true));
}
...
fn skill_context_message_with_wrong_parent_is_not_folded() {
    ...
    assert_eq!(turns[0].session_events.len(), 1);
    assert_eq!(turns[0].session_events[0].skill_context_folded, Some(false));
    assert!(
        turns[1]
            .user_message
            .as_deref()
